...
type private MzMLReaderMIRIMTransactionScope2() =
...
type MzMLReaderMIRIM2(filePath: string) =
    ...
    static member private getBinaryDataTypeOfP1D (peakArray:Peak1DArray) (keys:seq<string>) =
        for key in keys do
            ...
            | "MS:1000521"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP1D peakArray BinaryDataType.Float32 keys
            // Float64
            | "MS:1000523"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP1D peakArray BinaryDataType.Float64 keys
            // Int32
            | "MS:1000519"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP1D peakArray BinaryDataType.Int32 keys
            // Int64
            | "MS:1000522"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP1D peakArray BinaryDataType.Int64 keys
            | _             ->  ()
    ...
    static member private getCompressionTypeOfP1D (peakArray:Peak1DArray) (keys:seq<string>) =
        for key in keys do
            ...
            | "MS:1000576"  ->  peakArray.RemoveItem(key)
                                ...
                                MzMLReaderMIRIM2.getBinaryDataTypeOfP1D peakArray keys
            // ZlibCompression
    ...
    static member private addCompressionTypeToPeak1DArray (peakArray:Peak1DArray) =
        ...
        |> MzMLReaderMIRIM2.getCompressionTypeOfP1D' peakArray
        peakArray
    ...
    static member private getBinaryDataTypeOfP2D (peakArray:Peak2DArray) (keys:string []) =
        for key in keys do
            ...
            // Float32
            | "MS:1000521"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP2D peakArray BinaryDataType.Float32 keys
            // Float64
            | "MS:1000523"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP2D peakArray BinaryDataType.Float64 keys
            // Int32
            | "MS:1000519"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP2D peakArray BinaryDataType.Int32 keys
            // Int64
            | "MS:1000522"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM2.getArrayTypeOfP2D peakArray BinaryDataType.Int64 keys
            | _             ->  ()
    ...
    static member private getCompressionTypeOfP2D (peakArray:Peak2DArray) (keys:string []) =
        for key in keys do
            ...
            | "MS:1000576"  ->  peakArray.RemoveItem(key)
                                peakArray.CompressionType <- BinaryDataCompressionType.NoCompression
                                MzMLReaderMIRIM2.getBinaryDataTypeOfP2D peakArray keys
            // ZlibCompression
            | "MS:1000574"  ->  peakArray.RemoveItem(key)
                                peakArray.CompressionType <- BinaryDataCompressionType.ZLib
                                MzMLReaderMIRIM2.getBinaryDataTypeOfP2D peakArray keys
    ...
    static member private addCompressionTypeToPeak2DArray (peakArray:Peak2DArray) =
        ...
        |> Seq.map (fun pair -> pair.Key)
        |> Array.ofSeq
        |> MzMLReaderMIRIM2.getCompressionTypeOfP2D peakArray
        peakArray
        ...
        if convertedPeaks.Head.Length <> 0 then
            ...
            | BinaryDataCompressionType.NoCompression ->
                ...
                | BinaryDataType.Float32 ->
                    let mzs =
                        ...
                        |> (fun bytes -> MzMLReaderMIRIM2.byteToSingles false bytes)
                    ...
                    | BinaryDataType.Float64 ->
                        let intensities =
                            Convert.FromBase64String(convertedPeaks.[0])
                            |> (fun bytes -> MzMLReaderMIRIM2.byteToDoubles false bytes)
            ...
            | BinaryDataCompressionType.ZLib ->
                ...
                | BinaryDataType.Float32 ->
                    let mzs =
                        Convert.FromBase64String(convertedPeaks.[1])
                        |> MzMLReaderMIRIM2.decompressZlib
                        |> (fun bytes -> MzMLReaderMIRIM2.byteToSingles false bytes)
                    match peakArray.IntensityDataType with
                    | BinaryDataType.Float32 ->
                        let intensities =
            ...
            | BinaryDataCompressionType.NumPress ->
                ...
                | BinaryDataType.Float32 ->
                    let mzs =
                        Convert.FromBase64String(convertedPeaks.[1])
                        |> MzMLReaderMIRIM2.decodeMZsByLin
                    match peakArray.IntensityDataType with
                    | BinaryDataType.Float32 ->
                        let intensities =
                            Convert.FromBase64String(convertedPeaks.[0])
                            |> MzMLReaderMIRIM2.decodeIntensitiesByPIC
                        let peaks = Array.map2 (fun mz int -> new Peak1D(float int, float mz)) mzs intensities
                        peakArray.Peaks <- ArrayWrapper(peaks)
                    | BinaryDataType.Float64 ->
                        let intensities =
        ...
        | BinaryDataCompressionType.ZLib ->
            ...
            | BinaryDataType.Float32 ->
                let rts =
                    Convert.FromBase64String(peaks.[1])
                    |> MzMLReaderMIRIM2.decompressZlib
                    |> (fun bytes -> MzMLReaderMIRIM2.byteToSingles true bytes)
                match peakArray.IntensityDataType with
                | BinaryDataType.Float32 ->
                    let intensities =
                        Convert.FromBase64String(peaks.[0])
                ...
                | BinaryDataType.Float64 ->
                    let intensities =
                        Convert.FromBase64String(peaks.[0])
                        |> MzMLReaderMIRIM2.decompressZlib
                        |> (fun bytes -> MzMLReaderMIRIM2.byteToDoubles true bytes)
                    let peaks = Array.map2 (fun rt int -> new Peak2D(int, -1., float rt)) rts intensities
                    peakArray.Peaks <- ArrayWrapper(peaks)
                    | _ -> failwith "No compelement Type"
            | BinaryDataType.Float64 ->
    ...
    member private this.createPeak1DArray(?xmlReader: XmlReader) =
        ...
        let rec loop dbProcRef cvParams peaks read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                match dbProcRef with
                | Some ref  ->  MzMLReaderMIRIM2.addCompressionTypeToPeak1DArray peakArray |> ignore
                | None      ->  MzMLReaderMIRIM2.addCompressionTypeToPeak1DArray peakArray |> ignore
                ...
                if readOp()=true then 
                    ...
                    match dbProcRef with
                    | Some ref  ->  
                        MzMLReaderMIRIM2.get1DPeaks peaks peakArray
                        peakArray
                    | None      ->  
                        MzMLReaderMIRIM2.get1DPeaks peaks peakArray
                        peakArray
        ...
        let rec loop dbProcRef cvParams peaks read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                if readOp()=true then loop dbProcRef cvParams peaks read
                    ...
                    match dbProcRef with
                    | Some ref  ->  MzMLReaderMIRIM2.addCompressionTypeToPeak2DArray peakArray
                                    |> MzMLReaderMIRIM2.get2DPeaks peaks
                                    peakArray
                    | None      ->  MzMLReaderMIRIM2.addCompressionTypeToPeak2DArray peakArray
                                    |> MzMLReaderMIRIM2.get2DPeaks peaks
                                    peakArray
        loop None () [] ()
        ...
        let rec loop id sourceRef dataProcRef cvParams userParams scans precs products read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                | "referenceableParamGroupRef"  -> 
                    loop id sourceRef dataProcRef cvParams userParams scans precs products (readOp() |> ignore)
                | "cvParam"                     ->
                    loop id sourceRef dataProcRef ((this.getCVParam readSubtree)::cvParams) userParams scans precs products (readOp() |> ignore)
                | "userParam"                   ->
                    loop id sourceRef dataProcRef cvParams ((this.getUserParam readSubtree)::userParams) scans precs products   (readOp() |> ignore)
                | "scanList"                    ->
                    loop id sourceRef dataProcRef cvParams userParams (this.getScanList readSubtree) precs products read
                | "precursorList"               ->
                    loop id sourceRef dataProcRef cvParams userParams scans (this.getPrecursorList readSubtree) products read
                | "productList"                 ->
                    loop id sourceRef dataProcRef cvParams userParams scans precs (this.getProductList readSubtree) read
                | "binaryDataArrayList"         ->
                    spectrum <- new MassSpectrum(id, (if dataProcRef.IsSome then dataProcRef.Value else null), precs, scans, products, if sourceRef.IsSome then sourceRef.Value else null)
    ...
    member private this.getMzIOModel() =
        this.ResetReader()
        MzMLReaderMIRIM2.accessMzMLElement reader |> ignore
        MzMLReaderMIRIM2.checkSchema reader
        let rec outerLoop acc =
    ...
    interface IMzIOIO with
        ...
        member this.SaveModel() =
            ...
            try
                MzIOJson.SaveJsonFile(this.model, MzMLReaderMIRIM2.GetModelFilePath(filePath))
            with
                | :? Exception as ex ->
        ...
        member this.BeginTransaction() =
            ...
            new MzMLReaderMIRIMTransactionScope2() :> ITransactionScope
    ...
    /// ----------------------------------------------------------------------------------------------------------------------------------
    ///             Extensions for parsing mean inverse reduced ion mobility in addition to the mz and intensity values. 
    /// ----------------------------------------------------------------------------------------------------------------------------------
    ...
    static member private getArrayTypeOfP1D (peakArray:Peak1DArray) (arrayType:BinaryDataType) (keys:seq<string>) =
        for key in keys do
            ...
            // this is an extension of the original reader that makes it possible to read mirim data
            | "MS:1003006" ->   peakArray.RemoveItem(key)
                                peakArray?MirimDataType <- arrayType
            | _            -> ()

    /// <summary>
    /// converts the <binary> element of a BinaryDataArray element to a 'T array with the given converters based on the compression method used
    /// </summary>
    /// <param name="compression">the compression of the encodedCompressedPeak</param>
    /// <param name="noCompressionDecoding">the decoding function used for uncompressed peaks</param>
    /// <param name="zLibDecoding"the decoding function used for peaks encoded with zLib></param>
    /// <param name="numpressDecoding">the decoding function used for peaks encoded with numpress</param>
    /// <param name="numpressZlibDecoding">the decoding function used for peaks encoded with zlib and numpress</param>
    /// <param name="encodedCompressedPeak">the peak (in string form) to decode</param>
    static member private convert_peak<'T> 
        (compression: BinaryDataCompressionType) 
        (noCompressionDecoding: string -> 'T array)
        (zLibDecoding: string -> 'T array)
        (numpressDecoding: string -> 'T array)
        (numpressZlibDecoding: string -> 'T array)
        (encodedCompressedPeak:string)  =
            match compression with
            | BinaryDataCompressionType.NoCompression ->
                encodedCompressedPeak
                |> noCompressionDecoding
            | BinaryDataCompressionType.ZLib ->
                encodedCompressedPeak
                |> zLibDecoding
            | BinaryDataCompressionType.NumPress ->
            ...
            | BinaryDataCompressionType.NumPressZLib ->
                encodedCompressedPeak
                |> numpressZlibDecoding
            | _ -> failwith "No compelement Type"

    /// <summary>
    /// Converts the <binary> element of a BinaryDataArray element encoding a mz peak encoded as 64bit float with the given compression.
    /// </summary>
    /// <param name="compression">the compression of the encodedCompressedPeak</param>
    /// <param name="encodedCompressedPeak">the peak (in string form) to decode</param>
    static member convert_mz_float64_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM2.convert_peak<float>
            compression
            (Convert.FromBase64String >> MzMLReaderMIRIM2.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decodeMZsByLin)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.decodeMZsByLin)
            encodedCompressedPeak

    /// <summary>
    /// Converts the <binary> element of a BinaryDataArray element encoding a mz peak encoded as 32bit float with the given compression.
    /// </summary>
    /// <param name="compression">the compression of the encodedCompressedPeak</param>
    /// <param name="encodedCompressedPeak">the peak (in string form) to decode</param>
    static member convert_mz_float32_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM2.convert_peak<float>
            compression
            (Convert.FromBase64String >> MzMLReaderMIRIM2.byteToSingles false >> Array.map float)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.byteToSingles false >> Array.map float)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decodeMZsByLin)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.decodeMZsByLin)
            encodedCompressedPeak

    /// <summary>
    /// Converts the <binary> element of a BinaryDataArray element encoding a intensity peak encoded as 64bit float with the given compression.
    /// This differs from convert_mz_float64_peak because different precision encodings are used for intensity and m/z
    /// </summary>
    /// <param name="compression">the compression of the encodedCompressedPeak</param>
    /// <param name="encodedCompressedPeak">the peak (in string form) to decode</param>
    static member convert_intensity_float64_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM2.convert_peak<float>
            compression
            (Convert.FromBase64String >> MzMLReaderMIRIM2.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decodeIntensitiesByPIC)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.decodeIntensitiesByPIC)
            encodedCompressedPeak
            
    /// <summary>
    /// Converts the <binary> element of a BinaryDataArray element encoding a intensity peak encoded as 32bit float with the given compression.
    /// This differs from convert_mz_float32_peak because different precision encodings are used for intensity and m/z
    /// </summary>
    /// <param name="compression">the compression of the encodedCompressedPeak</param>
    /// <param name="encodedCompressedPeak">the peak (in string form) to decode</param>
    static member convert_intensity_float32_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM2.convert_peak<float>
            compression
            (Convert.FromBase64String >> MzMLReaderMIRIM2.byteToSingles false >> Array.map float)
            (Convert.FromBase64String >> MzMLReaderMIRIM2.decompressZlib >> MzMLReaderMIRIM2.byteToSingles false >> Array.map float)
    ...
    /// <summary>
    /// Converts the given convertedPeaks based on the compression and data types of the given Peak1DArray.
    /// In addition to mz and intensity peak1D in the Peak1DArray's `Peaks` property, also adds a dynamic member `Mirim` 
    /// containing the mean inverse reduced ion mobility data. Fails if there is no `MirimDataType` property on the Peak1DArray and if the convertedPeaks list does not contain 3 data arrays.
    /// 
    /// This function (equivalently to `get1DPeaks`) assumes fixed positions of the binary data arrays:
    /// 
    /// mz -> sourceFile: position 0, reader: position 2
    ///
    /// intensity -> sourceFile: position 1, reader: position 1
    ///
    /// mz -> sourceFile: position 2, reader: position 0
    /// 
    /// The indices are reversed in the reader because of the recursive functions used.
    ///
    /// This method (as the original) is not a pure function. it mutates the input peakArray, which revieves new properties on the members `Peaks`(mz/intensity data) and `?Mirim`(mean inverse reduced ion mobility data)
    ///
    /// </summary>
    /// <param name="convertedPeaks">the peaks (in string form) to decode and add to peakArray</param>
    /// <param name="peakArray">the Peak1DArray to recieve new properties.</param>
    static member private get1DPeaksWithMIRIM (convertedPeaks:string list) (peakArray:Peak1DArray) =

        if convertedPeaks.Length = 3 && (peakArray.GetDynamicMemberNames() |> Seq.contains "MirimDataType") then
            let mzs = 
                match peakArray.MzDataType with
                | BinaryDataType.Float32 -> MzMLReaderMIRIM2.convert_mz_float32_peak peakArray.CompressionType convertedPeaks.[2]
                | BinaryDataType.Float64 -> MzMLReaderMIRIM2.convert_mz_float64_peak peakArray.CompressionType convertedPeaks.[2]
                | _ -> failwith "No compelement Type"

            let intensities =
                match peakArray.IntensityDataType with
                | BinaryDataType.Float32 -> MzMLReaderMIRIM2.convert_intensity_float32_peak peakArray.CompressionType convertedPeaks.[1]
                | BinaryDataType.Float64 -> MzMLReaderMIRIM2.convert_intensity_float64_peak peakArray.CompressionType convertedPeaks.[1]
                | _ -> failwith "No compelement Type"
            ...
            let ionMobilities = 
                ...
                | BinaryDataType.Float32 -> MzMLReaderMIRIM2.convert_mz_float32_peak peakArray.CompressionType convertedPeaks.[0]
                | BinaryDataType.Float64 -> MzMLReaderMIRIM2.convert_mz_float64_peak peakArray.CompressionType convertedPeaks.[0]
                | _ -> failwith "No compelement Type"
            let peaks = Array.map2 (fun mz int -> new Peak1D(int, float mz)) mzs intensities
            peakArray.Peaks <- ArrayWrapper(peaks)
            peakArray?Mirim <- ionMobilities

        else 
            failwith "data list has no mean inverse reduced ion mobility (MIRIM) array."

    /// <summary>
    /// Reads the children of a BinaryArrayList element and creates a Peak1DArray with properties 
    /// `Peaks`(mz/intensity data) and `?Mirim`(mean inverse reduced ion mobility data)
    /// </summary>
    member private this.createPeak1DArrayWithMIRIM(?xmlReader: XmlReader) =
        let xmlReader = defaultArg xmlReader reader
        let readSubtree = xmlReader.ReadSubtree()
        let readOp = readSubtree.Read
        let peakArray = new Peak1DArray()
        let rec loop dbProcRef cvParams peaks read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                // add cvparams and user params as dynamic properties to the peakArray
                | "cvParam"                     ->  loop dbProcRef (peakArray.AddCvParam(this.getCVParam readSubtree)) peaks (readOp() |> ignore)
                | "userParam"                   ->  loop dbProcRef (peakArray.AddUserParam(this.getUserParam readSubtree)) peaks (readOp() |> ignore)
                
                // extract binary data encoded as string in the <binary> tag of a BinaryDataArray element
                | "binary"                      ->  loop dbProcRef cvParams ((this.getBinary readSubtree)::peaks) read
                | _                             ->  loop dbProcRef cvParams peaks (readOp() |> ignore)
            elif readSubtree.NodeType= XmlNodeType.EndElement && readSubtree.Name="binaryDataArray" then

                // dbprocref does not seem to matter at all, but i will not touch this legacy code more than i have to.
                match dbProcRef with

                // addCompressionTypeToPeak1DArray does more than the name suggests, as it also sets the Array data types on the peak1Darray (via 3 (!) chained method calls).
                | Some ref  ->  MzMLReaderMIRIM2.addCompressionTypeToPeak1DArray peakArray |> ignore
                | None      ->  MzMLReaderMIRIM2.addCompressionTypeToPeak1DArray peakArray |> ignore
                loop dbProcRef cvParams peaks (readOp() |> ignore)
            else
                if readOp()=true then 
                    ...
                    // again, dbprocref does not seem to matter at all.
                    match dbProcRef with
                    | Some ref  ->  
                        MzMLReaderMIRIM2.get1DPeaksWithMIRIM peaks peakArray
                        peakArray
                    | None      ->  
                        MzMLReaderMIRIM2.get1DPeaksWithMIRIM peaks peakArray
                        peakArray
    ...
    /// <summary> 
    /// Tries to parse Peak1DArray object containing mz, intensity, and mean inverse reduce ion mobility data from spectrum with the given id.
    /// </summary>
    member private this.tryGetPeak1DArrayWithMIRIM(spectrumID: string, ?xmlReader: XmlReader) =
        let xmlReader = defaultArg xmlReader reader
        let readSubtree =
            let rec loop acc =
                if xmlReader.NodeType=XmlNodeType.Element && xmlReader.Name="spectrum" then
        ...
        let rec loop read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                | "spectrum"            ->  if (this.getAttribute ("id", readSubtree)) = spectrumID then
                                                ...
                                                None
                | "binaryDataArrayList" ->  Some (this.createPeak1DArrayWithMIRIM readSubtree)
                |   _                   ->  loop (readOp() |> ignore)
            else
                if readOp()=true then loop read
                else None
    ...
    /// Creates Peak1DArray object containing mz, intensity, and mean inverse reduce ion mobility data from spectrum with the given id.
    /// Leaves the reader in a state that allows sequential reading of spectra.
    member this.getSpecificPeak1DArraySequentialWithMIRIM(spectrumID:string) =
        match reader.Name with
        | "spectrum" -> ()
        | _ -> 
            if not (reader.ReadToFollowing("spectrumList")) then
        ...
        let rec read peak1DArrayOption =
            ...
            | None ->
                ...
                // Creates a new reader based on the snapshot
                let nodeReader = XmlReader.Create (new StringReader(outerXml))
                let peak1DArrayOption' = this.tryGetPeak1DArrayWithMIRIM (spectrumID, nodeReader)
                nodeReader.Close()
                read peak1DArrayOption'
        read None