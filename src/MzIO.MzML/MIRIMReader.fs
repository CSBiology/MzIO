...
type private MzMLReaderMIRIMTransactionScope() =
...
type MzMLReaderMIRIM(filePath: string) =
    ...
    static member private getBinaryDataTypeOfP1D (peakArray:Peak1DArray) (keys:seq<string>) =
        for key in keys do
            ...
            | "MS:1000521"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP1D peakArray BinaryDataType.Float32 keys
            // Float64
            | "MS:1000523"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP1D peakArray BinaryDataType.Float64 keys
            // Int32
            | "MS:1000519"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP1D peakArray BinaryDataType.Int32 keys
            // Int64
            | "MS:1000522"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP1D peakArray BinaryDataType.Int64 keys
            | _             ->  ()
    ...
    static member private getCompressionTypeOfP1D (peakArray:Peak1DArray) (keys:seq<string>) =
        for key in keys do
            ...
            | "MS:1000576"  ->  peakArray.RemoveItem(key)
                                ...
                                MzMLReaderMIRIM.getBinaryDataTypeOfP1D peakArray keys
            // ZlibCompression
    ...
    static member private addCompressionTypeToPeak1DArray (peakArray:Peak1DArray) =
        ...
        |> MzMLReaderMIRIM.getCompressionTypeOfP1D' peakArray
        peakArray
    ...
    static member private getBinaryDataTypeOfP2D (peakArray:Peak2DArray) (keys:string []) =
        for key in keys do
            ...
            // Float32
            | "MS:1000521"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP2D peakArray BinaryDataType.Float32 keys
            // Float64
            | "MS:1000523"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP2D peakArray BinaryDataType.Float64 keys
            // Int32
            | "MS:1000519"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP2D peakArray BinaryDataType.Int32 keys
            // Int64
            | "MS:1000522"  ->  peakArray.RemoveItem(key)
                                MzMLReaderMIRIM.getArrayTypeOfP2D peakArray BinaryDataType.Int64 keys
            | _             ->  ()
    ...
    static member private getCompressionTypeOfP2D (peakArray:Peak2DArray) (keys:string []) =
        for key in keys do
            ...
            | "MS:1000576"  ->  peakArray.RemoveItem(key)
                                peakArray.CompressionType <- BinaryDataCompressionType.NoCompression
                                MzMLReaderMIRIM.getBinaryDataTypeOfP2D peakArray keys
            // ZlibCompression
            | "MS:1000574"  ->  peakArray.RemoveItem(key)
                                peakArray.CompressionType <- BinaryDataCompressionType.ZLib
                                MzMLReaderMIRIM.getBinaryDataTypeOfP2D peakArray keys
    ...
    static member private addCompressionTypeToPeak2DArray (peakArray:Peak2DArray) =
        ...
        |> Seq.map (fun pair -> pair.Key)
        |> Array.ofSeq
        |> MzMLReaderMIRIM.getCompressionTypeOfP2D peakArray
        peakArray
        ...
        if convertedPeaks.Head.Length <> 0 then
            ...
            | BinaryDataCompressionType.NoCompression ->
                ...
                | BinaryDataType.Float32 ->
                    let mzs =
                        ...
                        |> (fun bytes -> MzMLReaderMIRIM.byteToSingles false bytes)
                    ...
                    | BinaryDataType.Float64 ->
                        let intensities =
                            Convert.FromBase64String(convertedPeaks.[0])
                            |> (fun bytes -> MzMLReaderMIRIM.byteToDoubles false bytes)
            ...
            | BinaryDataCompressionType.ZLib ->
                ...
                | BinaryDataType.Float32 ->
                    let mzs =
                        Convert.FromBase64String(convertedPeaks.[1])
                        |> MzMLReaderMIRIM.decompressZlib
                        |> (fun bytes -> MzMLReaderMIRIM.byteToSingles false bytes)
                    match peakArray.IntensityDataType with
                    | BinaryDataType.Float32 ->
                        let intensities =
            ...
            | BinaryDataCompressionType.NumPress ->
                ...
                | BinaryDataType.Float32 ->
                    let mzs =
                        Convert.FromBase64String(convertedPeaks.[1])
                        |> MzMLReaderMIRIM.decodeMZsByLin
                    match peakArray.IntensityDataType with
                    | BinaryDataType.Float32 ->
                        let intensities =
                            Convert.FromBase64String(convertedPeaks.[0])
                            |> MzMLReaderMIRIM.decodeIntensitiesByPIC
                        let peaks = Array.map2 (fun mz int -> new Peak1D(float int, float mz)) mzs intensities
                        peakArray.Peaks <- ArrayWrapper(peaks)
                    | BinaryDataType.Float64 ->
                        let intensities =
        ...
        | BinaryDataCompressionType.ZLib ->
            ...
            | BinaryDataType.Float32 ->
                let rts =
                    Convert.FromBase64String(peaks.[1])
                    |> MzMLReaderMIRIM.decompressZlib
                    |> (fun bytes -> MzMLReaderMIRIM.byteToSingles true bytes)
                match peakArray.IntensityDataType with
                | BinaryDataType.Float32 ->
                    let intensities =
                        Convert.FromBase64String(peaks.[0])
                ...
                | BinaryDataType.Float64 ->
                    let intensities =
                        Convert.FromBase64String(peaks.[0])
                        |> MzMLReaderMIRIM.decompressZlib
                        |> (fun bytes -> MzMLReaderMIRIM.byteToDoubles true bytes)
                    let peaks = Array.map2 (fun rt int -> new Peak2D(int, -1., float rt)) rts intensities
                    peakArray.Peaks <- ArrayWrapper(peaks)
                    | _ -> failwith "No compelement Type"
            | BinaryDataType.Float64 ->
    ...
    member private this.createPeak1DArray(?xmlReader: XmlReader) =
        ...
        let rec loop dbProcRef cvParams peaks read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                match dbProcRef with
                | Some ref  ->  MzMLReaderMIRIM.addCompressionTypeToPeak1DArray peakArray |> ignore
                | None      ->  MzMLReaderMIRIM.addCompressionTypeToPeak1DArray peakArray |> ignore
                ...
                if readOp()=true then 
                    ...
                    match dbProcRef with
                    | Some ref  ->  
                        MzMLReaderMIRIM.get1DPeaks peaks peakArray
                        peakArray
                    | None      ->  
                        MzMLReaderMIRIM.get1DPeaks peaks peakArray
                        peakArray
        ...
        let rec loop dbProcRef cvParams peaks read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                if readOp()=true then loop dbProcRef cvParams peaks read
                    ...
                    match dbProcRef with
                    | Some ref  ->  MzMLReaderMIRIM.addCompressionTypeToPeak2DArray peakArray
                                    |> MzMLReaderMIRIM.get2DPeaks peaks
                                    peakArray
                    | None      ->  MzMLReaderMIRIM.addCompressionTypeToPeak2DArray peakArray
                                    |> MzMLReaderMIRIM.get2DPeaks peaks
                                    peakArray
        loop None () [] ()
    ...
    member private this.getMzIOModel() =
        this.ResetReader()
        MzMLReaderMIRIM.accessMzMLElement reader |> ignore
        MzMLReaderMIRIM.checkSchema reader
        let rec outerLoop acc =
    ...
    interface IMzIOIO with
        ...
        member this.SaveModel() =
            ...
            try
                MzIOJson.SaveJsonFile(this.model, MzMLReaderMIRIM.GetModelFilePath(filePath))
            with
                | :? Exception as ex ->
        ...
        member this.BeginTransaction() =
            ...
            new MzMLReaderMIRIMTransactionScope() :> ITransactionScope
    ...
    static member convert_mz_float64_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM.convert_peak<float>
            compression
            (Convert.FromBase64String >> MzMLReaderMIRIM.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decompressZlib >> MzMLReaderMIRIM.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decodeMZsByLin)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decompressZlib >> MzMLReaderMIRIM.decodeMZsByLin)
            encodedCompressedPeak
    ...
    static member convert_mz_float32_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM.convert_peak<float>
            compression
            (Convert.FromBase64String >> MzMLReaderMIRIM.byteToSingles false >> Array.map float)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decompressZlib >> MzMLReaderMIRIM.byteToSingles false >> Array.map float)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decodeMZsByLin)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decompressZlib >> MzMLReaderMIRIM.decodeMZsByLin)
            encodedCompressedPeak
    ...
    static member convert_intensity_float64_peak (compression: BinaryDataCompressionType) (encodedCompressedPeak:string) =
        MzMLReaderMIRIM.convert_peak<float>
            ...
            (Convert.FromBase64String >> MzMLReaderMIRIM.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decompressZlib >> MzMLReaderMIRIM.byteToDoubles false)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decodeIntensitiesByPIC)
            (Convert.FromBase64String >> MzMLReaderMIRIM.decompressZlib >> MzMLReaderMIRIM.decodeIntensitiesByPIC)
            encodedCompressedPeak
        ...
        let rec loop dbProcRef cvParams peaks read =
            if readSubtree.NodeType=XmlNodeType.Element then
                ...
                if readOp()=true then 
                    ...
                    match dbProcRef with
                    | Some ref  ->  
                        MzMLReaderMIRIM.get1DPeaksWithMIRIM peaks peakArray
                        peakArray
                    | None      ->  
                        MzMLReaderMIRIM.get1DPeaksWithMIRIM peaks peakArray
                        peakArray
