...
module AccessDB = 
    ...
    let insertModifiedSpectrumBy (spectrumPeaksModifierF: IMzIODataReader -> MassSpectrum -> string -> Peak1DArray ) (db: MzSQL) runID (reader:IMzIODataReader) (compress: string) (spectrum: MassSpectrum) = 
        let modifiedP = 
            spectrumPeaksModifierF reader spectrum compress
        db.InsertMass(runID, spectrum, modifiedP)
